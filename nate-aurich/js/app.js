...
function displayPics(){
  ...
  /**
   * This line threw an error because the variable 'rando' was created using 'let' inside of the while loop beginning on
   * line 31. 'let' only creates a block scoped variable. To fix this, I also moved the console log into the same
   * block.
   */
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes ${allProducts[i].views} views`);
    }
  }
